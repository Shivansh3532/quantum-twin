...
describe("deterministic core", () => {
  ...
  test("selection rejects failures and prefers fewer RSA signatures", () => {
    const candidate = (strategy: "direct"|"bridge", eligible: boolean, rsa: number) => ({ strategy, branch: strategy, threadId: "t", generationStatus: eligible ? "eligible" as const : "gate_failed" as const, worktreeCommit: "c", diffSha256: "d", changedLines: strategy === "direct" ? 5 : 10, gates: [{ name: "all", passed: eligible, detail: "" }], measurements: { rsaSignatures: rsa, envelopeBytes: 1, signMedianMs: 1, signP95Ms: 1, verifyMedianMs: 1, verifyP95Ms: 1 } });
