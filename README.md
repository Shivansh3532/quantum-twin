...
- **Codex/GPT use:** two authenticated `@openai/codex-sdk@0.144.6` builders use a Codex model — `gpt-5-codex` by default so ChatGPT-account `codex login` works; set `QT_MODEL` to override (e.g. `gpt-5.6-sol` on API-billing accounts). GPT classifies and explains evidence, while deterministic TypeScript alone controls gates, eligibility, and selection.
