...
export default function SupportPage() {
  return <><a className="skip-link" href="#main-content">Skip to main content</a><main id="main-content" tabIndex={-1}>
    <Nav current="/support"/>
    <section className="intro compact-intro"><div><p className="eyebrow">SUPPORTED CAPABILITIES</p><h1>Claims follow proof.</h1><p className="lede"><strong>Any Node.js service using RSA or ECDSA is in scope</strong> — primitives that secure thousands of npm packages. Quantum Twin covers five quantum-vulnerable boundary classes today (RSA, ECDSA, ECDH/DH, Web Crypto, JWT).</p><p className="lede">A row is <strong>fully supported</strong> only when it has a detector, an automatic adapter, an independent verifier, positive and negative crypto tests, a sample test environment, Windows and Ubuntu proof, and documentation. Anything missing keeps a row experimental or discovery-only — never dressed up as more.</p></div></section>
