...
type LabState = "idle" | "validating-url" | "cloning" | "importing" | "analyzing" | "supported" | "unsupported" | "review-required" | "blocked" | "ready" | "authorizing" | "building" | "evaluating" | "selected" | "no-safe-winner" | "failed";
...
export default function RepositoryLab({ onReport }: { onReport: (report: RunReport) => void }) {
  ...
  async function importAndAnalyze(event: React.FormEvent) {
    ...
    try {
      ...
      const next = inspected.status === "ready" ? "ready" : inspected.status === "review-required" ? "review-required" : inspected.report.supported.length ? "blocked" : "unsupported";
  ...
  return <div className="repository-lab">
    ...
    {analysis && <div className="lab-analysis"><h3>Analysis-only result</h3><dl><dt>System</dt><dd>{analysis.systemBundle.name}</dd><dt>Repository</dt><dd>{analysis.report.repository.name}</dd><dt>Source</dt><dd>{analysis.report.repository.source}</dd><dt>Resolved commit</dt><dd>{analysis.report.repository.resolvedCommit ?? "Not available for local import"}</dd><dt>Tree SHA-256</dt><dd>{analysis.systemBundle.repositories[0]?.treeSha256}</dd><dt>Runtime shape</dt><dd>{analysis.report.language.join(" + ") || "Unknown"} · {analysis.report.moduleType} · {analysis.report.packageManager}</dd><dt>Generated contract</dt><dd>{analysis.generatedContract.sha256} · review required</dd><dt>System graph</dt><dd>{analysis.systemBundle.graph.sha256} · {analysis.systemBundle.graph.nodes.length} nodes · {analysis.systemBundle.graph.edges.length} edges</dd><dt>Declared legacy contract</dt><dd>{analysis.contract.valid ? `valid · ${analysis.contract.sha256}` : "not required for analysis"}</dd></dl>
      ...
      {analysis.skippedFiles.length > 0 && <details open><summary>Skipped non-source analysis artifacts ({analysis.skippedFiles.length})</summary><ul className="lab-findings">{analysis.skippedFiles.map(file => <li key={file.path}><code>{file.path}</code><span>{file.size} bytes</span><small>{file.reason}</small></li>)}</ul></details>}
      ...
      {!analysis.contract.review && <details open className="contract-review"><summary>Review generated system contract and provenance</summary><div className="contract-grid"><div><strong>Commands</strong>{Object.entries(analysis.generatedContract.commands.value).map(([name, parts]) => <code key={name}>{name}: {parts.join(" ")}</code>)}</div><div><strong>Writable paths</strong>{analysis.generatedContract.writablePaths.value.map(item => <code key={item}>{item}</code>)}</div><div><strong>Protected paths</strong>{analysis.generatedContract.protectedPaths.value.map(item => <code key={item}>{item}</code>)}</div><div><strong>Entry points</strong>{analysis.generatedContract.entryPoints.value.map(item => <code key={item}>{item}</code>)}</div><div><strong>Boundaries</strong>{analysis.generatedContract.boundaries.value.map(item => <code key={item}>{item}</code>)}</div><div><strong>Provenance</strong><code>commands: {analysis.generatedContract.commands.provenance}</code><code>repositories: {analysis.generatedContract.repositories.provenance}</code><code>policy: {analysis.generatedContract.forbiddenOperations.provenance}</code></div></div></details>}
