...
export default function Dashboard({ recorded = false, initialReport = null }: { recorded?: boolean; initialReport?: RunReport | null }) {
  ...
  return <main>
    ...
    <section id="top" className="intro"><div><p className="eyebrow">EVIDENCE-BACKED POST-QUANTUM MIGRATION</p><h1>Two migrations enter.<br/><i>External proof decides.</i></h1><p className="lede">Quantum Twin gives identical repository contracts to two isolated Codex builders, then deterministic gates select only a candidate that satisfies declared compatibility.</p><div className="badges"><Badge tone={recorded ? "recorded" : "live"}>{recorded ? "Recorded Verified Run" : "Live Local"}</Badge><Badge>Node.js 24 · native RSA</Badge></div></div><aside><span className="index">01</span><p>Scanner identifies cryptography. Coding agents create competing patches. Quantum Twin independently proves eligibility.</p></aside></section>
