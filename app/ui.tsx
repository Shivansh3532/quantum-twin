...
export default function Dashboard() {
  const [report, setReport] = useState<Report>(null), [busy, setBusy] = useState(false), [error, setError] = useState("");
  const load = () => fetch("/api/runs/latest").then(r => r.ok ? r.json() : null).then(setReport).catch(() => setReport(null));
  useEffect(() => { load(); }, []);
  async function run() { setBusy(true); setError(""); try { const r = await fetch("/api/runs", { method: "POST" }); const data = await r.json(); if (!r.ok) throw new Error(data.error); setReport(data); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } }
